...
class TrainDP3Workspace:
    ...
    def get_policy_and_runner(self, cfg, checkpoint_num=3000):
        ...
        if not cfg.policy.use_pc_color:
            ckpt_file = pathlib.Path(f'./policy/3D-Diffusion-Policy/checkpoints/{self.cfg.task.name}_{cfg.training.seed}/{checkpoint_num}.ckpt')
        else:
            ckpt_file = pathlib.Path(f'./policy/3D-Diffusion-Policy/checkpoints/{self.cfg.task.name}_w_rgb_{cfg.training.seed}/{checkpoint_num}.ckpt')
